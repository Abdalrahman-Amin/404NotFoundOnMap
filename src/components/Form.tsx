  ...
  switch (action.type) {
    ...
    case "SET_ERROR": {
      return {
        ...
        isLoadingGeocoding: action.payload ? false : state.isLoadingGeocoding,
        cityName: action.payload ? "" : state.cityName,
        countryName: action.payload ? "" : state.countryName,
        emoji: action.payload ? "" : state.emoji,
        date: action.payload ? new Date() : state.date,
...
function Form() {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    dispatch({ type: "SET_DATE", payload: new Date(e.target.value) });
  };
